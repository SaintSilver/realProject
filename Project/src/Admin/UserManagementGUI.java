...
public class UserManagementGUI {
	...
	private void initialize() {
		...
		id_Field.setBackground(Color.LIGHT_GRAY);
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if ((!RadioButton_man.isSelected()) && (!RadioButton_woman.isSelected())) {
					JOptionPane.showMessageDialog(null, "성별을 선택하세요.");
				} else if(!(dao.isHangulSyllables(name_textField.getText()))){
					JOptionPane.showMessageDialog(null, "정확한 이름을 입력하세요.");
				}else if(!dao.checkInputOnlyNumber(contact_textField.getText())){
					JOptionPane.showMessageDialog(null, "정확한 번호를 입력하세요.");
				}else {
					...
					UserVO user = new UserVO(Integer.parseInt(num_textField.getText()), type_textField.getText(),
							id_Field.getText(), name_textField.getText(), contact_textField.getText(), gender);
